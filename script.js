...
(function () {
    ...
    // 寻找一个在桌面和手机端都存在的、可靠的挂载点
    const anchor = document.querySelector('#chat-buttons-container, #send_form'); // 优先用手机端的容器
    if (anchor) {
        anchor.prepend(carrotButton); // 将按钮插入到容器的开头
    ...
    carrotButton.addEventListener('click', (e) => {
        if (!isDragging) {
            e.stopPropagation(); inputPanel.classList.contains('active') ? hidePanel() : showPanel();
        }
        isDragging = false; // Reset after click
    });
    ...
    function dragHandler(e) {
        e.preventDefault();
        const initialX = carrotButton.offsetLeft; const initialY = carrotButton.offsetTop;
        ...
        function move(e) {
            ...
            const moveY = e.type.includes('mouse') ? e.pageY : e.touches[0].pageY;
            const deltaX = moveX - startX; const deltaY = moveY - startY;
            if (Math.abs(deltaX) > 5 || Math.abs(deltaY) > 5) { // Threshold to detect drag vs click
                hasMoved = true;
                isDragging = true;
                carrotButton.classList.add('is-dragging');
            }
            if (isDragging) {
                let newLeft = initialX + deltaX; let newTop = initialY + deltaY;
        ...
        function end() {
            ...
            if (hasMoved) {
                localStorage.setItem('cip_button_position_v2', JSON.stringify({ top: carrotButton.style.top, left: carrotButton.style.left }));
            }
            // Use a short timeout to set isDragging to false, allowing click to be suppressed
    ...
    function loadButtonPosition() {
        const savedPos = JSON.parse(localStorage.getItem('cip_button_position_v2'));
        if (savedPos?.top && savedPos?.left) { 
            carrotButton.style.position = 'fixed';
            carrotButton.style.top = savedPos.top; carrotButton.style.left = savedPos.left;
        }
    }
